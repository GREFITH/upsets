...
class HarvestUser(Base):
    ...
    access_roles: Mapped[list] = mapped_column(JSONB, nullable=False, server_default=text("'[]'::jsonb"))
    permissions_claims: Mapped[dict] = mapped_column(JSONB, nullable=False, server_default=text("'{}'::jsonb"))
    roles: Mapped[list] = mapped_column(JSONB, nullable=False, server_default=text("'[]'::jsonb"))
    
    created_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), server_default=text("now()"))
    updated_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), server_default=text("now()"))


class FreelancerAssignment(Base):
    __tablename__ = "freelancer_assignments"

    id: Mapped[uuid.UUID] = mapped_column(PGUUID(as_uuid=True), primary_key=True, default=uuid4)
    harvest_user_id: Mapped[int] = mapped_column(
        BigInteger, ForeignKey("harvest_users.harvest_id", ondelete="CASCADE"), nullable=False
    )
    ...
    bill_rate: Mapped[float | None] = mapped_column(Numeric)
    flat_rate_amount: Mapped[float | None] = mapped_column(Numeric)
    negotiated_hours: Mapped[float | None] = mapped_column(Numeric)
    start_date: Mapped[date] = mapped_column(Date, nullable=False)
    end_date: Mapped[date] = mapped_column(Date, nullable=False)
    monthly_overrides: Mapped[dict] = mapped_column(JSONB, nullable=False, server_default=text("'{}'::jsonb"))
    notes: Mapped[str | None] = mapped_column(Text)
    created_by: Mapped[str] = mapped_column(Text, nullable=False, server_default=text("'dashboard'"))
