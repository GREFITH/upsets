...
def _uses_connection_pool_unsafe_for_asyncpg(database_url: str) -> bool:
    ...
    Transaction-pooler URLs (:6543, hostname contains pooler) are the usual case. We also treat
    any *.supabase.co host as unsafe with QueuePool: parallel requests can still trigger
    DuplicatePreparedStatementError when the pool hands out connections that share server-side state.
    """
